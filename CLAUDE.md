...
- `backend/` — Node/Express API (CommonJS, Express 5). Talks to Supabase; sends email via the Brevo API (`lib/mailer.js` is the single transport).
...
node scripts/test-connections.js    # verify Supabase + Brevo creds and schema
...
Required env vars in `backend/.env` (see `scripts/test-connections.js` for the full list): `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `BREVO_API_KEY`, `OFFICE_EMAIL`, `PORT`.
