...
- `backend/` — Node/Express API (CommonJS, Express 5). Talks to Supabase; sends email via SendGrid API.
...
node scripts/test-connections.js    # verify Supabase + SendGrid creds and schema
...
Required env vars in `backend/.env` (see `scripts/test-connections.js` for the full list): `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `SENDGRID_API_KEY`, `GMAIL_USER` (used as from address), `OFFICE_EMAIL`, `PORT`.
