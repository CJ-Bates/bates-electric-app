...
// TOCTOU gap.
//
// opts: { visitId, appointmentAt (Date), bookedBy (display name),
//         assignedTechId? (uuid) }
// Returns { visit, previousAppointmentAt } — visit includes the
// subscription+customer join the office email path needs; null visit means
// not found / already completed / not assigned to that tech.

const { supabaseAdmin } = require('./supabase');

async function scheduleServiceVisit({ visitId, appointmentAt, bookedBy, assignedTechId }) {
  ...
  let preQuery = supabaseAdmin
    ...
    .select('id, appointment_at, status, assigned_tech_id')
    .eq('id', visitId);
  ...
  if (!before) return { visit: null, previousAppointmentAt: null };
  ...
  return { visit: updated || null, previousAppointmentAt: before.appointment_at || null };
}
