...
const FROM_EMAIL = process.env.GENERATOR_DIGEST_FROM || 'inspections.bateselectric@gmail.com';
...
router.post('/daily-email', requireCronSecret, async (req, res) => {
 ...
 // Also pull any failed-charge add-ons so we can surface them.
    const { data: failedAddons } = await supabaseAdmin
      .from('generator_pending_addons')
      .select('id, addon_type, amount_cents, notes, date_charged, subscription:generator_subscriptions(customer:generator_customers(name, phone))')
      .eq('status', 'failed')
      .order('created_at', { ascending: false });
    const failed = failedAddons || [];

    if (overdue.length === 0 && upcoming.length === 0 && failed.length === 0) {
 // Quiet morning  -  no email
 return res.json({ ok: true, sent: false, reason: 'No visits in the next 14 days', overdue: 0, upcoming: 0 });
 }

 const { subject, html, text } = buildEmail({ overdue, upcoming, failed, todayStr });
 ...
 res.json({ ok: true, sent: true, recipients: TO_EMAILS, overdue: overdue.length, upcoming: upcoming.length, failed: failed.length });
 } catch (err) {
...
function buildEmail({ overdue, upcoming, failed = [], todayStr }) {
 ...
 const addonLabelFor = (t) => ({
      ...
      const failedSection = (rows) => `
        <div style="margin-top:1.5rem;">
          <h2 style="color:#DC2626;font-size:1rem;margin-bottom:0.5rem;border-bottom:2px solid #DC2626;padding-bottom:0.25rem;">
            Failed charges (${rows.length}) - needs attention
          </h2>
          ${rows.map(a => {
            const c = (a.subscription && a.subscription.customer) || {};
            const amt = a.amount_cents ? '
...
<body style="margin:0;padding:24px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#111827;background:#f9fafb;">
 ...
 ${section('Due in next 14 days', upcoming, '#1F3A5F')}
          ${failed.length ? failedSection(failed) : ''}
 ...
 if (failed.length) {
        textLines.push('');
        textLines.push(`FAILED CHARGES (${failed.length}):`);
...
module.exports = router;
 + (a.amount_cents/100).toFixed(2) : '';
            const label = addonLabelFor(a.addon_type);
            return `<div style="padding:0.5rem 0;border-bottom:1px solid #eee;font-size:0.9rem;">
              <strong>${(c.name || 'Unknown')}</strong> - ${label} - ${amt}
              ${a.notes ? `<div style="color:#6b7280;font-size:0.8rem;">${(a.notes || '').replace(/</g,'&lt;')}</div>` : ''}
              ${c.phone ? `<div style="color:#6b7280;font-size:0.8rem;">${c.phone}</div>` : ''}
            </div>`;
          }).join('')}
        </div>
      `;
