...
// Hit by an external scheduler â protected by a shared secret instead of user JWT.
...
function buildEmail({ overdue, upcoming, todayStr }) {
  ...
  const rowHtml = (s, kind) => {
    ...
    const dueText = kind === 'overdue'
      ...
      : (d === 0 ? 'Today' : d === 1 ? 'Tomorrow' : `In ${d} days Â· ${fmtDate(s.next_visit_due)}`);
    ...
    return `
      <tr>
        <td style="padding:10px 12px;border-bottom:1px solid #e5e7eb;vertical-align:top;">
          <div style="font-weight:600;color:#1F3A5F;font-size:14px;">${escapeHtml(c.name || 'â')}</div>
          <div style="color:#6b7280;font-size:12px;">${escapeHtml(addr)} Â· ${escapeHtml(c.phone || '')}</div>
        </td>
        <td style="padding:10px 12px;border-bottom:1px solid #e5e7eb;vertical-align:top;color:#374151;font-size:13px;">
          ...
          <span style="color:#6b7280;font-size:12px;">${escapeHtml(s.gen_model || 'model n/a')} Â· ${escapeHtml(planLabel(s.plan))}</span>
        </td>
...
<body style="margin:0;padding:24px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#111827;background:#f9fafb;">
  <div style="max-width:680px;margin:0 auto;">
    <div style="background:#1F3A5F;color:#fff;padding:20px 24px;border-radius:8px 8px 0 0;">
      <h1 style="margin:0;font-size:20px;">Generator Care â Good Morning</h1>
    ...
    <div style="background:#fff;padding:20px 24px;border-radius:0 0 8px 8px;border:1px solid #e5e7eb;border-top:none;">
      ...
      <p style="margin:24px 0 0;text-align:center;">
        <a href="${dashboardUrl}" style="display:inline-block;background:#1F3A5F;color:#fff;text-decoration:none;padding:10px 20px;border-radius:6px;font-weight:600;font-size:14px;">Open Generator Care dashboard â</a>
      </p>
  ...
  // Plain-text fallback
  const textLines = [];
  textLines.push(`Generator Care â daily digest`);
  ...
  if (overdue.length) {
    ...
    for (const s of overdue) {
      const c = s.customer || {};
      const d = daysUntil(s.next_visit_due);
      textLines.push(`- ${c.name} â ${genClassLabel(s.gen_class)} â ${Math.abs(d)} days overdue â ${c.phone || ''}`);
    }
  }
  if (upcoming.length) {
    textLines.push('');
    ...
    for (const s of upcoming) {
      const c = s.customer || {};
      const d = daysUntil(s.next_visit_due);
      textLines.push(`- ${c.name} â ${genClassLabel(s.gen_class)} â ${d === 0 ? 'today' : d === 1 ? 'tomorrow' : `in ${d} days`} (${fmtDate(s.next_visit_due)}) â ${c.phone || ''}`);
    }
  }
  textLines.push('');
